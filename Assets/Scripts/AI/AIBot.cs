...
public class AIBot : NetworkBehaviour
{
    ...
    public void InitializeBot(string name, Transform player)
    {
        ...
        if (realModelPrefab != null) { humanoidModel = Object.Instantiate(realModelPrefab, transform); humanoidModel.name = "RealSoldierMesh"; humanoidModel.transform.localPosition = Vector3.zero; animator = humanoidModel.GetComponent<Animator>(); if (animator == null) animator = humanoidModel.AddComponent<Animator>(); rightArmTransform = humanoidModel.transform.Find("RightArm") ?? humanoidModel.transform; }
